...
func main() {
	...
	l := tLog{"s",log.New(os.Stdout,"tLog:",1)}
	l.Println(l.a)
	for  {
		select {
		case t := <- t.C:{
...
func build() {
	...
	var test  job.DeployJob;
	test.Config = sshConfig
	test.RemoteBefore = append(test.RemoteBefore,&job.RemoteComm{false,"ls"})
	test.RemoteBefore = append(test.RemoteBefore,&job.RemoteComm{false,"java -version"})
	test.Deploy()
