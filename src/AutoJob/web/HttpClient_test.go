...
func TestHttpDial(t *testing.T)  {
	...
	args["cityId"] = "36"
	res := httpDial("http://m-piao.-.com/rest/search","POST",head,args)
	t.Log(res)
	if res == "" {
		t.Error("res is null")
	}
...
func TestHttpDialBody(t *testing.T)  {
	...
	testURL := "http://gw.piao.-.com:171/"
	proURL := "http://gw.piao.-.com/"
	//updateProduct.action getCityInfo.action
	localURL := "http://gw.piao.-.com/"
...
func Benchmark_HttpDial(b *testing.B) {
	for i:=0;i<b.N ;i++{
		...
		head["Content-Type"] = "application/json;charset=UTF-8"
		args := make(map[string]string)
		//res := httpDial("http://h5.ticket.-.com//rest/cmsShop","GET",head,args)
		res := httpDial("http://h5.ticket.-.com/rest/allCity","GET",head,args)
		_ = res
		//b.Log(res)
	}
}
